...
public class MyJobListener implements JobExecutionListener {
    ...
    private ExportEvent exportEvent;
    ...
    public void beforeJob(JobExecution jobExecution) {
        ...
        processFilters(exportEvent.getFilters(),   );
   ...
   /* public Map<String, Object> fetchSavedFilters(Map<String, Object> filters) {
        Long userConfigId = null;
        Object value = filters.remove("userConfigId");
        if(value != null) {
            userConfigId = Long.parseLong(value.toString());
            String pageId = Constants.USER_CONFIGURATION.PRODUCT_PAGE_ID;
            value = filters.remove("pageId");
            if (value != null) {
                pageId = value.toString();
            }
            UserConfigurationUtil.UserConfigResponse userConfigResponse = userConfigurationUtil.fetchUserConfigurations(pageId, userConfigId);
            Object flagObject = filters.remove("ignoreUserConfigFilters");

            // if only false then need to apply filters fetched from database, also null will be considered as true
            if(flagObject != null && ! Boolean.valueOf(flagObject.toString()))
                filters = getDefaultFiltersFetchedFromDb(userConfigResponse);
            filters.put("userConfig", userConfigResponse);
        }
